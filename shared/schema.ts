import { pgTable, text, serial, integer, boolean, timestamp, real, date, index, foreignKey, uniqueIndex, varchar, jsonb } from "drizzle-orm/pg-core";
...
export const userCycles = pgTable("user_cycles", {
  ...
  return {
    ...
    userCycleUnique: uniqueIndex("user_cycle_unique_idx").on(table.userId, table.cycleId),
  };
});
...
export const teamCycles = pgTable("team_cycles", {
  id: serial("id").primaryKey(),
  ...
  return {
    ...
    teamCycleUnique: uniqueIndex("team_cycle_unique_idx").on(table.teamId, table.cycleId),
  };
});
...
export const objectives = pgTable("objectives", {
  ...
  return {
    ...
    parentObjectiveIdIdx: index("objectives_parent_objective_id_idx").on(table.parentObjectiveId),
...
export const meetingAgendaItems = pgTable("meeting_agenda_items", {
  ...
  return {
    meetingIdIdx: index("meeting_agenda_items_meeting_id_idx").on(table.meetingId),
    assigneeIdIdx: index("meeting_agenda_items_assignee_id_idx").on(table.assigneeId),
  };
});
...
export const financialData = pgTable("financial_data", {
  ...
  return {
    objectiveIdIdx: index("financial_data_objective_id_idx").on(table.objectiveId),
    uploadedByIdIdx: index("financial_data_uploaded_by_id_idx").on(table.uploadedById),
    teamIdIdx: index("financial_data_team_id_idx").on(table.teamId),
    dateIdx: index("financial_data_date_idx").on(table.date),
    quarterYearIdx: index("financial_data_quarter_year_idx").on(table.quarter, table.year),
  };
});
...
export const notifications = pgTable("notifications", {
  ...
  return {
    ...
    entityTypeIdIdx: index("notifications_entity_type_id_idx").on(table.entityType, table.entityId),
...
export const userSettings = pgTable("user_settings", {
  ...
  reminderFrequency: varchar("reminder_frequency", { length: 20 }).default("daily"),
...
export const activityLogs = pgTable("activity_logs", {
  ...
  return {
    ...
    entityTypeIdIdx: index("activity_logs_entity_type_id_idx").on(table.entityType, table.entityId),
...
export const integrations = pgTable("integrations", {
  ...
  return {
    ...
    createdByIdIdx: index("integrations_created_by_id_idx").on(table.createdById),
  };
});
